...
public class SwaggerConfig {
	...
	public Docket swaggerApi1() {
		...
		if (flagHeaderIsObrigatorio) {
			aParameterBuilder.name(HeaderAplicacaoEnum.HEADER_CODIGO_AUTORIZACAO.getValor()).modelRef(new ModelRef("string")).parameterType("header")
					.required(true).build();
		...
		return new Docket(DocumentationType.SWAGGER_2)
				...
				.groupName("v1")
				.select()
					.apis(RequestHandlerSelectors.basePackage("com.porto.portocom.portal.cliente.v1.controller"))
