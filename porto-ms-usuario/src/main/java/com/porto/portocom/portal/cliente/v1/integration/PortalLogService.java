...
public class PortalLogService implements IPortalLogService {
	...
	@Value("${integration.portallog.url.v1.salvarLogIntegracao}")
	private String salvarLogIntegracao;
	...
	private void enviaLogIntegracao(LogIntegracao logIntegracao) throws LogIntegracaoException {
		if (isFile) {
			...
			ResponseEntity<String> response = new RestTemplate().exchange(url+salvarLogIntegracao, HttpMethod.POST,
